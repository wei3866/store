	    ...
	    <div class="modal-content">
	      ...
	      <form class="form-horizontal">
			  ...
			  <div class="form-group">
			    ...
			    <div class="col-sm-10">
			      <input type="text" name="user" class="form-control" id="user_input" placeholder="姓名">
			  ...
			  <div class="form-group">
			    ...
			    <div class="col-sm-10">
			      <input type="text" name="number" class="form-control" id="number_input" placeholder="手机号码">
			      <span class="help-block"></span>
			    </div>
			  </div>
			...
			<div class="form-group">
				...
				<div class="col-sm-3">
					<select class="form-control" name="dId" id="a_select">
						<option value="">省</option>
					</select>
				</div>
				<div class="col-sm-3">
					<select class="form-control" name="dId" id="b_select">
						<option value="">市</option>
					</select>
				</div>
				<div class="col-sm-3">
					<select class="form-control" name="dId" id="c_select">
			...
			<div class="form-group">
			    ...
			    <div class="col-sm-10">
			      <input type="hidden" name="rReceiving"/>
			      <input type="text" name="xiangxi" class="form-control" id="xiangxi_Update_input" placeholder="5-10位字符">
			      <span class="help-block"></span>
			    </div>
			  </div>
	      ...
	      <div class="modal-footer">
	        ...
	        <button type="button" class="btn btn-primary" id="site_modal">确定</button>
	      </div>
	    </div>
	  </div>
	</div>
						...
						<c:forEach items="${sessionScope.ShoppingCart.items }" var="item">
							<tr class="${item.commodity.cId }">
								...
								<td>
									...
									<input type="text" value="${item.quantity }" size="1" id="${item.quantity }" class="text">
...
<script type="text/javascript">
		...
		//发送Ajax请求,把地址信息显示到下拉框中(省)
		function getDepts() {
			$.each(site,function(){
				$("#a_select").append("<option value='"+ this.name +"'>"+ this.name +"</option>")
			})
		}
		...
		$("#a_select").change(function(){
				$("#b_select option:not(:first)").remove();
				a_val = $(this).val();
					...
					$.each(site,function(){
						if (this.name == a_val) {
							$.each(this.city,function(){
								$("#b_select").append("<option value='"+ this.name +"'>"+ this.name +"</option>")
							})
						}
					})
				}
		...
		$("#b_select").change(function(){
				$("#c_select option:not(:first)").remove();
				var b_val = $(this).val();
				...
				if(b_val != ""){
					$.each(site,function(){
						if (this.name == a_val) {
							$.each(this.city,function(){
								if (this.name == b_val) {
									$.each(this.districtAndCounty,function(){
										$("#c_select").append("<option value='"+ this +"'>"+ this +"</option>")
									})
								}
							})
						}
		...
		$("#site_modal").click(function(){
			...
			if(!number_site() == false){
				return;
			}
			if(!xiangxi_site() == false){
				return;
			}
			...
			var c = $("#c_select").val();
			c = $.trim(c);
			var e = $("#xiangxi_Update_input").val();
			e = $.trim(e);
		...
		function user_site(){
			...
			if (!regName.test(user)) {
				show_validate_msg("#user_input", "error","用户名2-4位中文!");
				return false;
			} else {
		...
		//验证联系方式
		function number_site(){
			
			var number = $("#number_input").val();
			var regName = /(^[0-9]{11}$)/;
			
			if (!regName.test(number)) {
				show_validate_msg("#number_input", "error","联系方式格式不正确!");
				return false;
			} else {
				show_validate_msg("#number_input", "success", "");
			}
		}
		
		//验证详细地址
		$("#xiangxi_Update_input").change(function() {
			xiangxi_site();
		})
		
		//验证详细地址
		function xiangxi_site(){
			
			var number = $("#xiangxi_Update_input").val();
			var regName = /(^[\u2E80-\u9FFF]{5,10})/;
			
			if (!regName.test(number)) {
				show_validate_msg("#xiangxi_Update_input", "error","请检查字符个数!");
				return false;
			} else {
				show_validate_msg("#xiangxi_Update_input", "success", "");
			}
		}
